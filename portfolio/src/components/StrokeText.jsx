...
const StrokeText = ({
  ...
  const rawId = useId();
  const resolvedFillMode = curve ? 'fade' : fillMode;
  const fillEnabled = resolvedFillMode !== 'none';
  const useWipe = fillEnabled && resolvedFillMode === 'wipe';
  ...
  useLayoutEffect(() => {
    if (curve) {
      setBox({ x: 0, y: 0, width: 800, height: 300 });
      return;
    }
  ...
  }, [characters, fontSize, fontWeight, letterSpacing, strokeWidth, curve]);
  ...
  }, [box, dash, drawDuration, fillDelay, stagger, ease, trigger, resolvedFillMode, reverse, scrollStart]);

  const viewBox = curve ? `0 0 720 200` : (box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontSize} 600 ${fontSize * 1.3}`);

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': curve ? '200px' : `${Math.round(fontSize * 1.3)}px` }}
      role="img"
      aria-label={String(text ?? '')}
    >
      <svg className="stroke-text__svg" viewBox={viewBox} preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        <defs>
          {resolvedFillMode === 'wipe' && box && (
            <clipPath id={wipeId} clipPathUnits="userSpaceOnUse">
              <rect ref={wipeRectRef} x={box.x} y={box.y} width="0" height={box.height} />
            </clipPath>
          )}
          {curve && (
            <path id={`path-${wipeId}`} d="M 10 180 Q 360 20 710 180" fill="none" />
          )}
        </defs>
        ...
        >
          {curve ? (
            <textPath href={`#path-${wipeId}`} startOffset="50%" textAnchor="middle">
              {characters.map((char, index) => (
                <tspan data-stroke-char key={`s-${index}`}>{char === ' ' ? '\u00A0' : char}</tspan>
              ))}
            </textPath>
          ) : (
            characters.map((char, index) => (
        ...
        <text
          ...
          stroke="none"
          style={fontStyle}
          clipPath={resolvedFillMode === 'wipe' && box ? `url(#${wipeId})` : undefined}
        >
          {curve ? (
            <textPath href={`#path-${wipeId}`} startOffset="50%" textAnchor="middle">
              {characters.map((char, index) => (
                <tspan data-fill-char key={`f-${index}`}>{char === ' ' ? '\u00A0' : char}</tspan>
              ))}
            </textPath>
          ) : (
            characters.map((char, index) => (
